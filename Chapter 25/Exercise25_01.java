...
1. To build this traversal, we need to look at a node's left and right child. Since we can only look at one at a time,
we will create a list (called nodeList) that keeps track of the nodes whose second child we need to investigate.
Since our BST is made up TreeNodes, this list should hold TreeNode objects.
2. We first need to check if the root has a value. If not, end the method. If it does, we will add root to nodeList.
3. While nodeList still contains nodes to search, we will grab the first item (called current), remove current from
the list, and print current.
4. Check to see if current has a left child. If so, add to nodeList.
5. Check to see if current has a right child. If so, add to nodeList.

height Algorithm():
For this method, we will 2 height methods. One has no parameters and will default to using root to determine the
height. The second has a TreeNode node parameter and will find the height of the BST from that respective node.
The second method will use recursion.
...
3. Otherwise, compare the height of the left child node to the height of the right child node. To find these heights you will need to recursively call the method.
 */
